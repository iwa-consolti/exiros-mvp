...
- **▶️ CHECKPOINT 2026-06-21 (LEER ESTO PRIMERO; supera al de 06-19 en lo que toca a código):**
  ...
  **SIGUIENTE (orden sugerido para sesión nueva):** (1) **2.2** `[BE]` foto multipart (reemplaza `photoPath='PENDING'` que dejé como seam) → quita el placeholder en `trips.service.ts`. (2) **2.4** `[WEB]` lista de viajes activos (consume el trip creado). (3) Sesión **Android dedicada** para 1.4→1.5→2.3 (arranca emulador con `~/Library/Android/sdk/emulator/emulator -avd Pixel_3a_API_34_extension_level_7_arm64-v8a`). Para probar el backend ya hecho: levantar Postgres + `node backend/dist/main.js`, sembrar un destino con UUID **v4 válido** (¡no `1111...`! la validación `@IsUUID` lo rechaza), y curl con header `x-app-key: dev-app-key-cambia-en-prod`.
