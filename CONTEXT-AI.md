...
**Sesión larga (Opus). Cerrado: FASE 3 COMPLETA (rastreo por lotes, 3.1–3.5) + Fase 1 y Fase 2 CONFIRMADAS por Rogelio. Todo verde, nada a medias.**

- **DÓNDE ESTÁ EL PROYECTO:** El **camino crítico del producto está cerrado de extremo a extremo**: la app Android captura GPS en 2º plano (Foreground Service), encola en Room, y un WorkManager sube lotes **GZIP** al backend; el backend los persiste con idempotencia por `batchId`; el portal web mueve el camión en el mapa por polling. **Fase 1 y Fase 2 = ✔ Confirmadas por Rogelio.** Fase 3 = código 100%, **pendiente ✔ humano**. Diagnóstico previo: H1/H2/H3 cerrados.
- **QUÉ SE HIZO ESTA SESIÓN (commits clave):** `345798a` migración renombrada a `2026_06_21_001_init` (formato legible) · `d62c040` 3.1 (Room + captura pasiva + restaura M3) · `6ceeffe` 3.2 (Foreground Service + notificación + distance filter + ActivityRecognition) · `530691d` 3.4 (ingesta lotes GZIP + idempotencia + selección geocerca) · `b14e332` 3.3 (WorkManager envío GZIP) · `ea16730` 3.5 (mapa W1 polling) + commits de confirmación Fase 1/2. Detalle por bloque en la bitácora de `PLAN.md` (2026-06-22).
- **CÓMO LEVANTAR TODO PARA PROBAR (orden):** (1) `docker compose -f infra/docker-compose.yml up -d` (Postgres). (2) `cd backend && npx prisma migrate deploy && npm run build && node dist/main.js` (:3000). (3) `cd web && npm run dev` (:5173). (4) Emulador: `~/Library/Android/sdk/emulator/emulator -avd Pixel_3a_API_34_extension_level_7_arm64-v8a -no-snapshot-save`; build+instalar: `cd android && JAVA_HOME=$(/usr/libexec/java_home -v 21) ./gradlew assembleDebug && ~/Library/Android/sdk/platform-tools/adb install -r app/build/outputs/apk/debug/app-debug.apk`. Para ver la BD: `cd backend && npx prisma studio`. Header móvil `x-app-key: dev-app-key-cambia-en-prod`; `10.0.2.2` = host desde el emulador.
- **⚠️ FOOTGUN DE PRUEBAS (RN-11):** `adb shell pm clear com.exiros.tracker` o "Olvidar viaje (debug)" borran el viaje **local** pero NO el del backend → el deviceId queda con un viaje EN_RUTA colgado y la app NO puede crear otro (409 RN-11). **Reset limpio:** `docker exec exiros-postgres psql -U exiros -d exiros -c "update \"Trip\" set status='CONCLUIDO' where status='EN_RUTA';"` antes de re-probar. Además: tras `pm clear`, esperar a que carguen los destinos antes de tocar "Iniciar viaje" (si no, validación falla en silencio).
- **HUECOS RESTANTES (priorizados):** **H4** `/api/web/*` sin Guard JWT (cualquiera lista viajes) → Fase 6.1. **Fase 4** cierre por geocerca (haversine sobre los `geofenceCandidates` que ya expone 3.4; el backend ya responde `stopTracking:false` placeholder). **H5** la web diverge del diseño: falta shell+sidebar (Mapa·Viajes·Destinos·Usuarios), W2 debe ser **tabla** (hoy tarjetas), W1 con KPIs/clusters → bloques WEB ~6.x. **H6** `/health` → Fase 9. **D-A** `api-spec.md` vs `openapi.yaml`. Menores: constant-time en `AppKeyGuard`; ingesta no exactamente-once si llega un punto nuevo entre reintentos (batchId cambia) — aceptable para MVP.
- **▶️ SIGUIENTE PASO (decisión de Rogelio):** opciones — **(A) Fase 4** (cierre automático por geocerca + cierres manuales M4/M5: el seam ya está, falta el haversine y las pantallas) · **(B) H4 JWT** en `/api/web/*` (seguridad, acotado) · **(C) bloques WEB** para alinear el portal al diseño (shell+sidebar+tabla). ⚠️ KPI "<10% batería" e hibernación por ActivityRecognition NO verificables sin teléfono físico (emulador = Android puro): nunca reportarlos cumplidos.
- **AVISO al abrir sesión nueva:** este checkpoint + `PLAN.md` bastan; NO re-explorar el repo. Verificar `git status` limpio antes de tomar bloque. Los ✔ de Fase 3 siguen "pendiente confirmar humano" hasta que Rogelio los valide.

## Historial de sesiones (resumen; detalle por bloque en la bitácora de `PLAN.md`)
- **2026-06-22 (sesión Fase 3, la de hoy):** Fase 3 completa (3.1 Room+captura · 3.2 Foreground Service+notif+distance filter+ActivityRecognition · 3.4 ingesta GZIP+idempotencia · 3.3 WorkManager · 3.5 mapa polling) + migración renombrada + Fase 1 y 2 confirmadas por Rogelio. → checkpoint vigente arriba.
