...
**Sesión larga (Opus). Cerrado: FASE 3 (rastreo por lotes) + FASE 4 (cierres) completas. Fase 1 y 2 ✔ confirmadas por Rogelio. Todo verde.**

- **DÓNDE ESTÁ:** Flujo de vida del viaje completo de extremo a extremo: crear (M2) → rastrear en 2º plano (Foreground Service + Room + WorkManager GZIP) → ver moverse en el mapa web (polling) → **cerrar** (automático por geocerca / operador M4 / admin web) → M5 + GPS off. **Fase 1+2 = ✔ confirmadas; Fase 3+4 = código 100%, pendiente ✔ humano.**
- **QUÉ SE HIZO ESTA SESIÓN (commits clave):** Fase 3: `d62c040` 3.1 · `6ceeffe` 3.2 · `530691d`+`a199001` 3.4 (ingesta GZIP, alineada al spec) · `b14e332` 3.3 · `ea16730` 3.5. Fase 4: `77a8bfb` 4.1 (geocerca auto) · `d8db7a7` 4.2-be+4.3 (cierre operador/admin) · `7df965f` 4.2-and+4.4 (M4/M5+stop GPS) · `b8668eb` 4.5 (forzar cierre web). + `ca05d46` loop de pruebas 30s. Detalle por bloque en bitácora `PLAN.md`.
...
- **HUECOS RESTANTES (priorizados):** **H4** `/api/web/*` sin Guard JWT → Fase 6.1 (y entonces `closedById` del cierre admin deja de ser null). **H5** la web diverge del diseño: shell+sidebar (Mapa·Viajes·Destinos·Usuarios), W2 = tabla (hoy tarjetas), W3 detalle completo (hoy solo "Forzar cierre"), W1 con KPIs/clusters → bloques WEB ~6.x. **H6** `/health` → Fase 9. Menores: constant-time en `AppKeyGuard`; ingesta no exactamente-once si llega un punto nuevo entre reintentos; cierre admin con `window.prompt` (MVP) y `fallbackToDestructiveMigration` en Room (caché local).
- **▶️ SIGUIENTE PASO:** orden acordado (cimientos→fachada). HECHO esta sesión (pend. confirmar humano): **6.1 JWT** · **7.1 export Excel** · **7.2 botón+filtros web** · **6.3 PARCIAL** (W0 login + sesión web, adelantado como enabler de 7.2). **SIGUIENTE:** **6.2** (CRUD usuarios + RolesGuard ADMIN) → **W5** (cierra 6.3, gestión de usuarios) → **Fase 5** (destinos CRUD + W4) → **bloques WEB (C)** shell+sidebar+tabla W2+detalle W3 con contenido real. ⚠️ KPI "<10% batería"/hibernación NO verificables sin teléfono físico: nunca reportarlos cumplidos. ⚠️ **H4 cerrado por 6.1**; **H5 sigue abierto** (portal aún es vista única sin shell/sidebar/routing).
