...
- **▶️ CHECKPOINT 2026-06-21 (LEER ESTO PRIMERO; supera al de 06-19 en lo que toca a código):**
  ...
  **ESTADO:** Fase 1 = 1.1/1.2/1.3 ✅, 1.4/1.5 pendientes (Android). Fase 2 = 2.1/2.2 ✅, 2.3/2.4 pendientes.
  **SIGUIENTE (orden sugerido para sesión nueva):** (1) **2.4** `[WEB]` lista/tarjetas de viajes activos — necesita primero un `GET /api/web/trips` (aún no existe; el espacio `/api/web/*` con Guard JWT no está montado, pero para la lista basta un endpoint de lectura) + portar tarjetas a React consumiendo `tripId/foto/status`. (2) Sesión **Android dedicada** para 1.4→1.5→2.3 (arranca emulador con `~/Library/Android/sdk/emulator/emulator -avd Pixel_3a_API_34_extension_level_7_arm64-v8a`). (3) Fase 3+ (rastreo/ingesta/cierre). Para probar el backend ya hecho: levantar Postgres + `node backend/dist/main.js`, sembrar un destino con UUID **v4 válido** (¡no `1111...`! la validación `@IsUUID` lo rechaza), y curl con header `x-app-key: dev-app-key-cambia-en-prod`.
