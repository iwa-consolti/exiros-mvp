...
> **✅ RESUELTAS:** **D1** → Opción A, **Kotlin nativo** (ADR-004, implementado en Fases 1–4). **D2** → túnel cloudflared para dev (bloque 9.1; `scripts/tunnel.sh`) y, **revisado 2026-06-25, deploy real en AWS EC2 free tier + RDS PostgreSQL + cloudflared en el EC2** (HTTPS) para la demo. Se conservan abajo por trazabilidad.
...
**I-03 — Rol "Super administrador" protegido.** *(Pro-mejora — NO desarrollar ahora)*
- **Idea:** un tercer rol `SUPER_ADMIN` por encima de Administrador, protegido (no se puede dar de baja ni cambiar de rol; siempre ≥1). Aparece en el prototipo Figma/`documentacion UX-UI.md`.
- **Por qué fuera del MVP (decisión Rogelio 2026-06-23):** el **documento fuente** (`docs/fuente/...alcance MVP...md` §4/§7.2 = única verdad) solo define **dos niveles**: monitorista y administrador. El 3er rol excedía la fuente → se **eliminó** (migración `2026_06_23_003_remove_super_admin_role`). El modelo quedó en 2 roles: `ADMIN`, `MONITOR`. Reintroducir si el cliente lo pide explícitamente.
...
**Despliegue (ver D2/ADR-009 — revisado 2026-06-25):**
- **Desarrollo:** backend local + **túnel** (cloudflared, `scripts/tunnel.sh`) para que el teléfono lo alcance.
- **Demo:** **AWS EC2 (free tier)** + **RDS PostgreSQL** + **cloudflared en el EC2** para la URL **HTTPS** (el APK release exige https; no hay dominio). En el `.env` del EC2: `TRUST_PROXY_IP=true` (rate-limit por IP real tras el túnel) y `WEB_ORIGIN` (CORS).
- **Web:** build Vite con `VITE_API_URL` apuntando a la URL del EC2 (sin esto pega a `localhost:3000`).
- **Salud:** `GET /api/health` (readiness con ping a DB) para monitor/auditor/cloudflared.
- **Fotos:** volumen local (MVP).
- **Logs/monitoreo:** logs estructurados de Nest. Sin observabilidad avanzada.
...
- [x] README de arranque (local) + guion de demo *(9.3; despliegue cloud pospuesto, solo túnel)*
- [ ] ADRs y specs actualizadas
- [ ] `main` limpia y verde para auditoría
- [x] Demo ensayada sin fallos *(9.3: 2 corridas e2e seguidas, cierre AUTO_GEOFENCE; pendiente confirmar humano)*
...
> **Nota de diseño GPS/ruta (2026-06-23):** no crear `Route` ni guardar coordenadas como array. La ruta del viaje es la secuencia `Trip 1:N Location`: cada punto guarda `tripId`, `lat`, `lng`, `accuracyMeters`, `recordedAt`, `receivedAt` y `batchId`. Para dibujar la polilínea, consultar `Location WHERE tripId = ? ORDER BY recordedAt ASC, id ASC`; `recordedAt` manda porque es la hora real de captura en el teléfono, `id` solo desempata si dos puntos tienen el mismo timestamp. El mapa W1 muestra el **último punto**; W3/detalle reconstruye la **ruta histórica** desde todos los puntos. Hueco a corregir antes de demo/auditoría: hacer el `batchId` estable en Android tipo outbox (asignarlo a los puntos del lote antes del primer envío y reintentar exactamente ese lote hasta ACK; puntos nuevos van a otro batch).
...
- [x] **5.1 `[BE]`** CRUD `/api/web/destinations`. **(pendiente confirmar humano)** *(2026-06-23: hecho en 10.5. `DestinationsService` extraído a `DestinationsModule` (compartido web+mobile, DRY); CRUD findAllWeb/create/update/setActive (baja lógica, no borra: viajes referencian destino). Controller `/api/web/destinations` GET/POST/PATCH/:id/deactivate|restore, protegido `JwtAuthGuard`+`AdminRolesGuard` (nuevo, solo ADMIN; doc UX §6.1 validar en backend). DTO radio 100-700. e2e +6 incl. 403 MONITOR.)*
...
- [x] **6.2 `[BE]`** CRUD usuarios + Guard de rol ADMIN. **(pendiente confirmar humano)** **Decisión vigente (2026-06-23):** **2 roles** `ADMIN` + `MONITOR` (la **fuente** §4/§7.2 solo define esos dos). El `SUPER_ADMIN` que se probó el 2026-06-22 se **eliminó** (migración `..._003_remove_super_admin_role`) por exceder la fuente → ver Pro-mejora I-03. Acceso a Usuarios/Destinos exclusivo de ADMIN (validar en backend vía `AdminRolesGuard`, no solo ocultar en UI). *(2026-06-23: hecho en 10.6. `UsersService` + controller `/api/web/users` (GET/POST/PATCH/:id/deactivate|restore) con `JwtAuthGuard`+`AdminRolesGuard`. Alta hashea password (bcrypt), email único → 409. Seed crea ADMIN. e2e (403 MONITOR / 400 pass corta / 201 / 409 dup / update+baja) → 46/46. *(Nota: el 2026-06-23 se construyó con `SUPER_ADMIN` protegido y se revirtió ese mismo día a 2 roles; ver decisión arriba e I-03.)*)*
- [x] **6.3 `[WEB]`** W0 login + rutas protegidas + W5 gestión usuarios. **Hecho cuando:** sin token no se entra; admin da de alta un monitorista que luego entra. **(pendiente confirmar humano)** *(2026-06-23: COMPLETO. W5 gestión de usuarios hecho en 10.6 (tabla + panel lateral de alta + modal "Dar de baja"); verificado Playwright: alta real de "María Pérez" monitorista. "¿olvidaste tu contraseña?" = modal informativo, fuera de MVP. Shell+routing = 10.1.)* *(2026-06-22: PARCIAL — adelantado como enabler de 7.2, que necesitaba sesión web. HECHO: W0 login (split-screen al diseño), token+usuario en localStorage, `Authorization` en toda llamada `/api/web/*`, gate de sesión (sin token → login), botón "Salir", 401→logout automático. Verificado en navegador (Playwright). FALTA para cerrar 6.3: **W5 gestión de usuarios** (depende de 6.2 CRUD usuarios) y "¿olvidaste tu contraseña?" (fuera de MVP). Routing real (react-router) no añadido: el portal es una sola vista; el shell+sidebar es bloque WEB posterior.)*
...
- [x] **9.3 `[DOC]`** README de arranque + ensayo de demo. **Hecho cuando:** la demo e2e corre 2 veces seguidas sin fallar. **(pendiente confirmar humano)** *(2026-06-23: hecho. README reescrito (estaba en "Fase 0/código pendiente") con: cómo levantar local (docker+backend+web+emulador), guion de demo (crear viaje → `route-sim.sh` → cierre AUTO_GEOFENCE en portal y M5 → export Excel), túnel para teléfono, gates, stack/estructura actualizados. **Ensayo e2e en emulador real: 2 corridas seguidas SIN fallar** — viaje nuevo → simulador → backend cierra AUTO_GEOFENCE → CONCLUIDO (corrida #1 ~30s, #2 ~10s). Gotcha de ensayo: mover GPS fuera de la geocerca antes de cada corrida (si no, arranca sobre el centro y cierra al instante).)*

#### Fase 10 — Portal web fiel al diseño (shell + 6 pantallas)
> Referencia obligatoria: `docs/exiros-reference-image/webapp/` + `documentacion UX-UI.md`. Replicar, no improvisar. Ver renderizado (Playwright) antes de entregar cada pantalla. **Decisiones vigentes:** **2 roles ADMIN/MONITOR** (la fuente solo define esos dos; `SUPER_ADMIN` del Figma eliminado, ver I-03) · estados-mapa derivados POSPUESTOS (solo En ruta/Concluido) · la **fuente** es la única verdad; si la app la excede, se ajusta.
...
- [x] **10.1 `[WEB]`** Shell: sidebar (logo + Mapa·Viajes·Destinos·Usuarios + Cerrar sesión; Destinos/Usuarios solo Admin) + header (nombre/rol) + routing (react-router) entre secciones sin recargar. Reubica el login dentro del shell. **Hecho cuando:** se navega entre secciones por el sidebar; sin token → login; Monitorista no ve Destinos/Usuarios. **(pendiente confirmar humano)** *(2026-06-23: hecho, commit `12b6d64`; iconos lucide-react en sidebar 2026-06-23.)*
- [x] **10.2 `[WEB]`** W1 Mapa "Monitoreo en tiempo real": KPIs (En ruta/Concluidos por ahora), buscador + filtros, capas (Mapa/Satélite/Geocercas/Clusters), clusters. **SIN export.** **(pendiente confirmar humano)** *(2026-06-22: hecho. KPIs clicables En ruta/Concluidos (filtran), buscador (placa/folio/proveedor) + filtros estado/destino/proveedor + Limpiar, `LayersControl` Mapa(OSM)/Satélite(Esri)/Geocercas, leyenda. Verificado Playwright. **DIFERIDO (decisión de Rogelio, no hacer sin avisar):** (a) KPIs/markers de estados derivados Detenido/Sin actualización/Cerca de destino — requieren umbrales que Rogelio difirió; (b) capa **Clusters** — optimización para alto volumen, omitida en MVP (react-leaflet-cluster con react19/RL5 = riesgo de build).)* **DIFERIDO LEVANTADO (2026-06-23, a pedido de Rogelio):** (a) estado **Detenido** = `EN_RUTA` sin lectura GPS >30 min (derivado en el portal en `tripState.ts`, no toca backend; los 3 estados son En ruta/Detenido/Concluido — Sin actualización/Cerca/Contingencia NO se usan por decisión de Rogelio); (b) capa **Clusters** con `leaflet.markercluster` vanilla (esquiva el riesgo de react-leaflet-cluster); (c) **leyenda 3 colores** + (d) **tabla "Viajes visibles en el mapa"** (placa/folio/proveedor/destino/estado/últ.actualización/ojo; fila→centra mapa). Gates verdes, verificado Playwright con 2 viajes sembrados (En ruta + Detenido).
...
- [x] **10.5 `[BE+WEB]`** W4 Destinos: tabla (Nombre/Centro/Radio/Maps/Estado/Acciones) + modal "Nuevo/Editar destino" con mapa para fijar pin + radio (máx 700 m) + baja/restaurar. **(pendiente confirmar humano)** *(2026-06-23: hecho. BE = Fase 5.1 (CRUD + AdminRolesGuard solo ADMIN). FE: tabla fiel + modal con mapa interactivo (clic fija el centro, círculo del radio). Verificado Playwright: alta real "Patio Vendedor Saltillo" aparece en la tabla. Gates: backend lint+build+e2e 41/41, web lint+build.)*
- [x] **10.6 `[BE+WEB]`** W5 Usuarios: tabla (Nombre/Correo/Rol/Estado/Acciones) + panel lateral "Nuevo usuario" + modal "Dar de baja". (2 roles ADMIN/MONITOR.) **(pendiente confirmar humano)** *(2026-06-23: hecho. BE = Fase 6.2 (UsersService + AdminRolesGuard, 2 roles). FE: tabla fiel + búsqueda + filtro rol + `UsuarioPanel` (slide-over alta/edición) + modal confirm "Dar de baja". Verificado Playwright: alta real de monitorista aparece en la tabla. Gates: backend e2e 46/46, web lint+build. Nota: se quitó el `SUPER_ADMIN`/candado el mismo día, ver 6.2 e I-03.)*
...
- **2026-06-23 (noche — pulido Mapa W1, levanta diferido de 10.2):** A pedido de Rogelio se completó el Mapa al mockup `mapa-webapp.png` con **3 estados** (decisión: NO usar Sin actualización/Cerca/Contingencia). Nuevo `web/src/tripState.ts` (estado **derivado en el portal**, sin tocar backend): `Detenido` = viaje EN_RUTA sin lectura GPS >30 min — encaja con la hibernación por acelerómetro (camión quieto → deja de reportar → lectura envejece). `TripsMap`: panel de capas Mapa/Satélite/Geocercas/**Clusters** (`leaflet.markercluster` vanilla, esquiva el riesgo de `react-leaflet-cluster`+React19), markers por color de estado, paneo al seleccionar. `MapaPage`: KPIs En ruta/Detenido/Concluido, leyenda 3 colores, **tabla "Viajes visibles en el mapa"** (fila→centra mapa, ojo→detalle). **Revisión de código aplicada:** 🔒 corregido **XSS almacenado** en popup de Clusters (campos de la app de acceso libre iban en HTML interpolado → ahora nodos DOM con `textContent`); 🔁 DRY `MAP_STATES` (array de estados repetido 3×→ fuente única). Gates verdes (lint+tsc+build); verificado Playwright con 2 viajes sembrados (En ruta + Detenido), consola limpia. Commit `f782da2` (+ fix de revisión). **Sembrados de prueba en BD dev:** `DEMO-ENRUTA`/`DEMO-DETEN` (borrar antes de la demo si molestan).
- **2026-06-23 (noche — hardening "casi producción", auditoría de Rogelio):** Triados 13 hallazgos; aplicado Batch A + #9 + #4 (resto aceptado/documentado). **#1** anti-lockout en `users.service` (no auto-baja, no auto-degradación, no degradar al último ADMIN activo; controller pasa `@CurrentUser`). **#3** fail-fast de secretos: `validateEnv` al arranque (DATABASE_URL/JWT_SECRET/TRIP_TOKEN_SECRET/APP_KEY) + `getOrThrow` en `auth.module` y `trips.service` (antes `?? ''`). **#7** radio de geocerca real en el mapa activo (listado expone `radiusMeters`; `api.ts`+`TripsMap` lo usan en vez de 300 fijo). **#9** cierre forzado web = **solo ADMIN** (`AdminRolesGuard` en el endpoint; decisión de Rogelio sobre el doc ambiguo). **#11** gate de lint **puro** (`lint` sin `--fix`; `lint:fix` aparte) + corregidas las violaciones Prettier del spec. **#12** comentarios obsoletos "Super admin" eliminados. **#4** cadencia GPS alineada a la fuente (captura 2 min/300 m, hibernación 5 min/500 m; antes 5 s/25 m) y eliminado el `startDebugSyncLoop` (debug 30 s). **Verificado:** backend lint puro+build+unit 21+e2e **47** (nuevo: monitor→cierre web 403); curl real (auto-baja/auto-demote→400); web lint+build; Android lintDebug+assembleDebug OK. **Aceptado/diferido:** #2 (JWT stateless, TTL 12h), #3-android (release usa 10.0.2.2/dev-key, no hay backend prod aún), #5 (mismo-timestamp raro), #6/#8/#10/#13 (menores). **Nota:** quitar el debug-sync hace que en emulador no se vean puntos cada 30 s; el cierre por geocerca sigue siendo inmediato (sync prioritario al entrar al radio).
- **2026-06-24 (madrugada — refactor + limpieza + UX + 2.ª ronda de hallazgos + docs):** Continuación de la auditoría. **Refactor (`3190c43`):** `backend/src/web/` plano → feature-folders (`trips/users/destinations/reports/`) con `git mv` (sin tocar lógica/rutas/payloads; lint puro cazó un import roto del e2e). **Limpieza (`6ce4ddf`):** modal en blanco por `color-scheme: light dark`→`light`; borrado scaffold muerto (app.controller/service + tests Hello World), assets Vite y `App.css` huérfanos, carpeta vacía `web/dto`. **Tests+orden (`c4813e5`):** 5 unit de invariantes de usuarios (#2) + orden de ruta determinístico `recordedAt+id` (#3) en backend (detalle+geocerca) y Room. **UX sidebar (`24d1443`,`5cd6e87`):** colapsable; tras feedback de Rogelio quedó como **rail de iconos** (72 px) con el toggle **dentro** de la barra (antes se cortaba). **3.ª ronda (`e1af323`):** #5 export por destino (listado expone `destination.id`; verificado e2e — un warning de React key resultó ser síntoma de `dist` viejo, se reconstruyó), #7 cierre admin con **modal** (no `window.prompt`, reusa `.modal-*`), #6 guard que falla el release Android con placeholders dev (debug intacto; `assembleRelease` falla, verificado). **Docs (`e934497`):** README ejecutable multiplataforma (npm install + `.env` + comandos backend/web/móvil debug+release) y `api-spec`/`openapi` con cierre web **solo ADMIN**. **DIFERIDO:** #4-idempotencia mismo-timestamp (migración Prisma + `clientPointId`). Gates verdes 3 capas; todo pusheado a `origin/main`. **Próximos pasos en CONTEXT-AI:** validación humana + smoke de cierres + limpiar BD dev + (opcional) `/health`.
- **2026-06-25 (despliegue real + rúbrica + ciberseguridad):** Sesión para dejar el repo listo para **prueba en teléfono físico** y subir rúbrica. **Decisión de deploy (revisa ADR-009):** AWS EC2 free tier + RDS + cloudflared (HTTPS); Rogelio lo monta. **Diagnóstico (no era bug):** "el viaje se cierra solo al iniciarlo" = el emulador quedaba sobre la geocerca tras `route-sim.sh` → AUTO_GEOFENCE correcto. Android limpio (DEV_AIDS off, sin mock-location). **Firma APK** (`af0f41f`): `signingConfigs.release` vía `android/keystore.properties` (gitignored); `assembleRelease` → APK firmado verificado. **Quick wins web** (`7bfe250`+`b18c469`): `getStoredUser()` defensivo, `<RequireAdmin>`, DRY (`constants.ts`/`format.ts`). **Auditoría backend** (`5475c07`): calidad alta sin deuda; añadido `/api/health` + `AppKeyGuard` constant-time. **Hardening del endpoint público** (`0026c23` — los auditores aprietan en ciberseguridad): foto con extensión por MIME (no `originalname` → no aloja `.html`), **`ProxyThrottlerGuard`** rate-limit por IP real (`CF-Connecting-IP`, solo si `TRUST_PROXY_IP=true`), login `@Throttle(10/min)`. Ya estaba fuerte: tripToken HMAC+sha256, anti-IDOR, DTOs acotados, helmet, Prisma parametrizado. **Docs** (`8246c51`/`a506c78`/`285afd3`): `.env.example` web+backend (PORT/WEB_ORIGIN/THROTTLE_*/TRUST_PROXY_IP/VITE_API_URL), `/health` en openapi, specs y ADRs (007/009/012) al día. **Checklist 1×1 en `PENDIENTES.md`.** **Hallazgos de barrido (afectan demo en prod):** el seed oficial NO crea destinos → crear 1–2 en el portal de prod o la app no inicia viajes; la web pega a `localhost:3000` sin `VITE_API_URL`. **Migraciones 002/003 (add/remove SUPER_ADMIN):** son un ida-y-vuelta cosmético; como ya se aplicaron al RDS, NO se colapsan (romperían el deploy) — limpieza diferida a un reset de prod real. Gates verdes 3 capas; pusheado.
