...
- **2.1 `[BE]`** `GET /api/mobile/destinations` + `POST /api/mobile/trips` (DTO, validación 7 campos, `clientRequestId` idempotente, tripToken derivado por HMAC y hasheado en BD, deviceId, RN-11, snapshot inmutable de geocerca). **Hecho cuando:** curl crea viaje "En ruta" y devuelve tripToken+geocerca; repetir exactamente la solicitud devuelve el mismo viaje/token y no duplica registros.
